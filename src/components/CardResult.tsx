    ...
    <Card key={uuidv4()} className="hover:shadow-md transition-shadow">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-1 text-xs text-muted-foreground">
            Coincidencia: {result.distance.toFixed(2)}
      ...
      <CardContent className="space-y-4">
        <div className="bg-muted/50 p-3 rounded-lg">
          <p className="text-sm leading-relaxed">{result.chunk}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="space-y-1 text-xs">
              {result.ddgfechasancion && (
                <div>
                  ...
                  {formatDate(result.ddgfechasancion)}
                </div>
              )}
              {result.ddgfechapublicacion && (
                <div>
                  <span className="font-medium">Publicación:</span>{" "}
                  {formatDate(result.ddgfechapublicacion)}
                </div>
              )}
              {result.ddgfechapromulgacion && (
                <div>
                  <span className="font-medium">Promulgación:</span>{" "}
                  {formatDate(result.ddgfechapromulgacion)}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="space-y-1 text-xs">
              ...
              <div>
                <span className="font-medium">Tipo publicación:</span>{" "}
